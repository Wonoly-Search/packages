...
class PiPackage {
    accepts(query) {
        if (query) {
            ...
            return (
                query.substring(0, 2) === "pi" ||
                query.substring(query.length - 3, query - 1) === "pi" ||
                query.substring(0, 1) === "pi" ||
                query.substring(query.length - 2, query - 1) === "π"
            );
        }
    }
    ...
    async render(query) {
        ...
        try {
            decimal =
                /^(?:pi|π)?\s*(?:to|first)?\s*(?<decimal>\d+)\s*(?:(?:decimal|digit)s?)?\s*(?:of\s+(?:pi|π))?$/i.exec(
                    query
                ).groups.decimal;
        } catch (_) {
            decimal = 5;
        ...
        return new Promise(function (resolve, reject) {
            resolve({
                html: `
                    <div id="wonoly__package__pi__wrapper">
                ...
                js: ``,
                css: `
                    #wonoly__package__pi__wrapper div:nth-child(2) {
                        font-weight: bold;
                        margin-bottom: 0.5rem;
                        line-break: anywhere;
                    }

                    #wonoly__package__pi__wrapper div:nth-child(3) {
                        font-weight: bold;
                        opacity: .7;
                    }
                `,
